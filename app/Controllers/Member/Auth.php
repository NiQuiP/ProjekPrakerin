...
class Auth extends BaseController
{
    ...
    public function login()
    {
        ...
        echo view('member/auth/login', $data);
    }
        ...
        if (!$rules) {
            ...
            if (empty($err)) {
                if ($memberInfo['is_verifikasi'] == 'no') {
                    ...
                    return redirect()->to('member/kirim_ulang');
                }
            }
            if (empty($err)) {
                $dataSesi = [
                ...
                if ($memberInfo['level'] == '1' or $memberInfo['level'] == '2') {
                    return redirect()->to('admin/dashboard');
                } else {
                    notif_swal('success', 'Selamat Datang');
                    return redirect()->to('member/my-profile');
                }
            }
    ...
    public function registerProcess()
    {
        ...
        if (!$rules) {
            ...
            $link = site_url("member/verifikasi/?email=$email&token=$token");
            $attachment = "";
            $to = "$email";
    ...
    function kirim_ulang_token()
    {
        ...
        $verifikasi = $this->request->getVar('verifikasi');
        if ($verifikasi) {
            return redirect()->to('member/verifikasi');
        }
    ...
    function index()
    {
        ...
        echo view('member/lengkapi_data', $data);
    }
    ...
    public function indexHandler()
    {
        ...
        $rules = $this->validate([
            ...
            'instansi' => [
                'rules' => 'required',
                'errors' => [
                    'required' => 'Instansi Pendidikan tidak boleh kosong'
                ]
            ],
            'nama_instansi' => [
                'rules' => 'required',
        ...
        if (!$rules) {
            ...
            return view('member/lengkapi_data', [
                'validation' => $this->validator->getErrors()
            ]);
    ...
    //         $email = $this->request->getVar('email');
    //         $member = new MemberModel();
    //         $memberInfo = $member->where($fieldType, $email)->first();
    //         $memberEmail = $memberInfo['email'];

    //         if ($fieldType == 'email') {
    //             if (!$fieldType == '') {
    //                 if ($memberEmail != $fieldType) {
    //                     $err = 'Email yang anda masukkan tidak terdaftar';
    //                 }
    //             } else {
    //                 $err = 'Email harus diisi untuk melakukan reset password';
    //             }
    ...
    //     }
    //     echo view('member/auth/forgetPassword');
    // }
    ...
    function resetPassword()
    {
        ...
        echo view('member/auth/resetPassword', $data);
    }
    ...
    function resetPasswordProcess()
    {
        ...
        if (empty($err)) {
            ...
            if (!$rules) {
                return view('member/auth/resetPassword', [
                    'validation' => $this->validator->getErrors()
                ]);
            } else {
                $member = new MemberModel();
